<script>
  ...
  export let color = 'var(--figure-path, #4594e3)';
  ...
  $: {
    ...
    sliderStyle = `background: linear-gradient(to right, ${color} ${fillPercent}%, var(--figure-flow, #d3d3d3) ${fillPercent}%)`;
  }
</script>
...
<style>
  ...
  .tick {
    ...
    width: 2px;
    height: 10px;
    background-color: var(--figure-muted, #7b7b7b);
